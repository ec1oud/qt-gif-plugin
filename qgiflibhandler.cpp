...
//		   image->size().width(), image->size().height());
	GifFileType* gifFile = DGifOpen(device(), doInput);
	if (!gifFile)
		return false;
...
//	memset(image->bits(), 0, image->numBytes());
	...
	do
	{
		...
		case IMAGE_DESC_RECORD_TYPE:
			...
			{
				qWarning("QGIFLibHandler::read: error %d", _GifError);
				return false;
			}
...
bool QGIFLibHandler::write ( const QImage & image )
{
	...
	int numColors = 1 << BitSize(toWrite.numColors());
...
//qDebug("color %d has %02X%02X%02X", c, qRed(colorTable[c]), qGreen(colorTable[c]), qBlue(colorTable[c]));
	...
	GifFileType * gif = EGifOpen(device(), doOutput);
...
//		qDebug("description is of qvariant type %d", descText.type());
	...
	if (EGifPutImageDesc(gif, 0, 0, toWrite.width(), toWrite.height(), 0, &cmap) == GIF_ERROR)
		qCritical("EGifPutImageDesc returned error %d", GifLastError());
